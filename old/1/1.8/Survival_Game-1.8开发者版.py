...
import time
import os
os.system("title Survival_Game 1.8")
os.system("color b")
screen = []
a = ""
b = ""
...
def is_number(s):
    ...
    try:
        ...
        pass
    return False#此函数源自https://www.runoob.com/python3/python3-step1.html
while True:
        ...
        if " ahfuiehaihfrhsufcha" in a:
            ...
            try:
                ...
                for i in range(100):
                    ...
                    os.system("cls")
                a = "true"
        elif " Slime Workingroom from liu" in a:
            a = a.replace(" Slime Workingroom from liu", "")
            b = a.split(" ")
            try:
                if b[5] == "nothing":
                    b[5] = ""
...
while True:
    ...
    if a == "exit":
        if house == "":
            house = "nothing"
        print("存档码:")
        a = str(day_money)+" "+str(money)+" "+str(job)+"#"+str(CLA)+"#"+str(TP)+"#"+str(IQ)+"#"+str(PT)
        b = " "+admin+" "+str(day)+" "+str(house)+" Slime Workingroom from liu"
        print(a + b)
        print("请复制上一行,不要多复制(最好粘贴在一个TXT里)")
        break
    time.sleep(1)
a = input()